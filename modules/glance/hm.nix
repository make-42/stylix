{ config, lib, ... }:
{
  options.stylix.targets.glance.enable = config.lib.stylix.mkEnableTarget "Glance" true;

  config = lib.mkIf (config.stylix.enable && config.stylix.targets.glance.enable) {
          ...
          let
            ...
            h =
              if delta == 0 then
                ...
                0;
            l = (max + min) / 2;
            s = if delta == 0 then 0 else delta / (100 - lib.max (2 * l - 100) (100 - (2 * l)));
          in
